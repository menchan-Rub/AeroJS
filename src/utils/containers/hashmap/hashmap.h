...
class HashMap {
 public:
  ...
  struct Entry {
    ...
    // デフォルトコンストラクタ
    Entry()
        : hash(0), dist(0), occupied(false), kv(key_type(), mapped_type()) {
    }

    // 値初期化コンストラクタ
    template <typename K, typename V>
    Entry(std::size_t h, std::size_t d, K&& k, V&& v)
        : hash(h), dist(d), occupied(true), kv(std::forward<K>(k), std::forward<V>(v)) {
    }

    // コピーコンストラクタ
    Entry(const Entry& other)
        : hash(other.hash), dist(other.dist), occupied(other.occupied), kv(other.kv) {
    }

    // ムーブコンストラクタ
    Entry(Entry&& other) noexcept
        : hash(other.hash), dist(other.dist), occupied(other.occupied), kv(std::move(other.kv)) {
  ...
  class iterator_base {
   ...
   private:
    ...
    iterator_base(EntryPtr ptr, EntryPtr end)
        : m_ptr(ptr), m_end(end) {
      // 空のエントリを飛ばす
