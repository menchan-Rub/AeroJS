...
public:
    ...
    // デフォルトコンストラクタ
    constexpr StringView() noexcept : m_view() {}

    // 文字列とサイズからの構築
    constexpr StringView(const char* str, size_type len) noexcept : m_view(str, len) {}

    // null終端文字列からの構築
    constexpr StringView(const char* str) noexcept : m_view(str) {}

    // std::stringからの構築
    StringView(const std::string& str) noexcept : m_view(str) {}

    // std::string_viewからの構築
    constexpr StringView(std::string_view view) noexcept : m_view(view) {}
    ...
    // 要素アクセス
    constexpr char operator[](size_type pos) const noexcept { return m_view[pos]; }
