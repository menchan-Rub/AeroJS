...
void MCPServer::sendInitialization(websocketpp::connection_hdl hdl) {
    json message = {
        ...
        {"id", generateId()},
        {"data", {
            {"serverName", m_options.serverName},
            {"version", m_options.version},
            {"requireAuth", m_options.enableAuthentication},
...
json MCPServer::handleScriptExecute(const std::string& clientId, const json& params) {
    try {
        ...
        std::string filename = params.contains("filename") && params["filename"].is_string() 
                               ? params["filename"].get<std::string>() : "script.js";
        ...
        if (result.contains("success") && result["success"].get<bool>()) {
            ...
            return createErrorResponse(
                ...
                {
                    {"error", {
                        {"name", result.contains("errorName") ? result["errorName"] : "Error"},
                        {"message", result.contains("errorMessage") ? result["errorMessage"] : "Unknown error"},
                        {"stack", result.contains("stack") ? result["stack"] : ""}
                    }}
                }
            );
        }
...
json MCPServer::handleMemoryGetUsage(const std::string& clientId, const json& params) {
    try {
        ...
        bool detailed = params.contains("detailed") && params["detailed"].is_boolean() 
                        ? params["detailed"].get<bool>() : false;
    ...
    try {
        ...
        if (!request.contains("type") || !request["type"].is_string()) {
            json errorResponse = {
                ...
                {"error", {
                    ...
                    {"message", "無効なリクエスト: typeフィールドがありません"}
                }}
            };
        ...
        if (!request.contains("params") || !request["params"].is_object()) {
            json errorResponse = {
                ...
                {"error", {
                    {"code", "INVALID_REQUEST"},
                    {"message", "無効なリクエスト: paramsフィールドがありません"}
                }}
            };
            return errorResponse.dump();
        }
        ...
        json errorResponse = {
            ...
            {"error", {
                {"code", "PARSE_ERROR"},
                {"message", std::string("JSONパースエラー: ") + e.what()}
            }}
        };
        return errorResponse.dump();
    } catch (const std::exception& e) {
        json errorResponse = {
            ...
            {"error", {
                {"code", "INTERNAL_ERROR"},
                {"message", std::string("内部エラー: ") + e.what()}
            }}
        };
        return errorResponse.dump();
    }
