...
namespace {
    ...
    constexpr auto DEFAULT_CACHE_TTL = seconds(3600);
    constexpr size_t DEFAULT_CACHE_SIZE = 1000;
    constexpr uint32_t DEFAULT_RETRY_COUNT = 3;
    constexpr auto DEFAULT_RETRY_DELAY = milliseconds(100);
...
public:
    ...
    std::string_view getParams() const noexcept { return m_params; }
    const ExecutionOptions& getOptions() const noexcept { return m_options; }
    ...
    // 実行結果の取得
    ToolResult* getResult() const noexcept { return m_result; }
    ...
    // エラー情報の取得
    std::exception_ptr getError() const noexcept { return m_error; }
...
public:
    explicit BatchExecutor(ToolManager& manager) : m_manager(manager) {}
...
public:
    explicit PluginManager(bool enabled = true) : m_enabled(enabled) {}
