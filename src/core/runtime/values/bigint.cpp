...
// デフォルトコンストラクタ (0)
BigInt::BigInt() : positive_(true), digits_(1, 0) {
}

// int64_t からのコンストラクタ
BigInt::BigInt(int64_t value) : positive_(value >= 0) {
...
// uint64_t からのコンストラクタ
BigInt::BigInt(uint64_t value) : positive_(true) {
    if (value == 0) {
        digits_.push_back(0);
    } else {
        while (value > 0) {
...
// 文字列からのコンストラクタ
BigInt::BigInt(const std::string& str, int radix) : positive_(true), digits_(1, 0) {
    if (radix < 2 || radix > 36) {
