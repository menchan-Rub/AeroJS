...
struct Property {
    ...
    // デフォルトコンストラクタ
    Property() : value(nullptr), flags(PropertyFlags::None), getter(nullptr), setter(nullptr) {}
    
    // 値プロパティ用コンストラクタ
    Property(Value* v, PropertyFlags f) : value(v), flags(f), getter(nullptr), setter(nullptr) {}
