...
ExecutionContext::ExecutionContext()
    ...
    , m_variableObject(nullptr)
    , m_strictMode(false)
    , m_hasConsole(true)
    , m_hasModules(true)
    , m_sharedArrayBuffersEnabled(false)
    , m_locale("en-US")
    , m_isRunning(false) {
    initialize();
}
...
void ExecutionContext::initializeGlobalFunctions(Object* globalObj) {
    // eval
    auto evalFunction = new NativeFunction(this, "eval", [this](const std::vector<Value>& args) -> Value {
    ...
    // isFinite
    auto isFiniteFunction = new NativeFunction(this, "isFinite", [this](const std::vector<Value>& args) -> Value {
        if (args.empty()) {
            return Value(false);
        }
    ...
    // isNaN
    auto isNaNFunction = new NativeFunction(this, "isNaN", [this](const std::vector<Value>& args) -> Value {
        if (args.empty()) {
            return Value(true);
        }
    ...
    // parseFloat
    auto parseFloatFunction = new NativeFunction(this, "parseFloat", [this](const std::vector<Value>& args) -> Value {
        if (args.empty()) {
    ...
    // parseInt
    auto parseIntFunction = new NativeFunction(this, "parseInt", [this](const std::vector<Value>& args) -> Value {
        if (args.empty()) {
            return Value(std::numeric_limits<double>::quiet_NaN());
        }
    ...
    // encodeURI
    auto encodeURIFunction = new NativeFunction(this, "encodeURI", [this](const std::vector<Value>& args) -> Value {
        if (args.empty()) {
            return Value("undefined");
        }
    ...
    // decodeURI
    auto decodeURIFunction = new NativeFunction(this, "decodeURI", [this](const std::vector<Value>& args) -> Value {
        if (args.empty()) {
            return Value("undefined");
        }
    ...
    // encodeURIComponent
    auto encodeURIComponentFunction = new NativeFunction(this, "encodeURIComponent", [this](const std::vector<Value>& args) -> Value {
        if (args.empty()) {
            return Value("undefined");
        }
    ...
    // decodeURIComponent
    auto decodeURIComponentFunction = new NativeFunction(this, "decodeURIComponent", [this](const std::vector<Value>& args) -> Value {
        if (args.empty()) {
            return Value("undefined");
        }
