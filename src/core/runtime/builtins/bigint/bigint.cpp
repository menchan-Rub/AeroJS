...
BigInt::BigInt() : m_sign(0), m_digits() {
    // 0を表すBigIntは空のディジット配列
}

BigInt::BigInt(int64_t value) : m_digits() {
    if (value == 0) {
        m_sign = 0;
        return;
    }
...
BigInt::BigInt(const std::string& str, int radix) : m_sign(0), m_digits() {
    if (radix < 2 || radix > 36) {
...
BigInt::BigInt(const BigInt& other) : m_sign(other.m_sign), m_digits(other.m_digits) {
}

BigInt::BigInt(BigInt&& other) noexcept : m_sign(other.m_sign), m_digits(std::move(other.m_digits)) {
    other.m_sign = 0;
}
...
BigIntObject::BigIntObject(BigInt value) : Object(), m_value(std::move(value)) {
}
