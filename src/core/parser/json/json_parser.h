...
public:
     ...
     * @return エラーの位置情報
     */
    const JsonErrorPosition& position() const { return position_; }
...
public:
     ...
     * @param value ブール値
     */
    JsonValue(bool value) : type_(JsonValueType::Boolean), bool_value_(value) {}
    
    /**
     * @brief 整数値のJSONValueを構築する
     * @param value 整数値
     */
    JsonValue(int value) : type_(JsonValueType::Number), number_value_(static_cast<double>(value)), is_integer_(true) {}
    
    /**
     * @brief 64ビット整数値のJSONValueを構築する
     * @param value 64ビット整数値
     */
    JsonValue(int64_t value) : type_(JsonValueType::Number), number_value_(static_cast<double>(value)), is_integer_(true) {}
    
    /**
     * @brief 浮動小数点値のJSONValueを構築する
     * @param value 浮動小数点値
     */
    JsonValue(double value) : type_(JsonValueType::Number), number_value_(value), is_integer_(false) {}
    
    /**
     * @brief 文字列のJSONValueを構築する
     * @param value 文字列値
     */
    JsonValue(const std::string& value) : type_(JsonValueType::String), string_value_(value) {}
    
    /**
     * @brief 文字列のJSONValueを構築する（右辺値参照）
     * @param value 文字列値
     */
    JsonValue(std::string&& value) : type_(JsonValueType::String), string_value_(std::move(value)) {}
     ...
     */
    bool isBool() const { return type_ == JsonValueType::Boolean; }
     ...
     */
    bool isInteger() const { return type_ == JsonValueType::Number && is_integer_; }
...
public:
     ...
     */
    JsonParser() : options_(JsonParserOptions()) {}
    
    /**
     * @brief 指定されたオプションでJSONパーサーを構築する
     * @param options パーサーオプション
     */
    explicit JsonParser(const JsonParserOptions& options) : options_(options) {}
     ...
     * @return パーサー統計情報
     */
    const JsonParserStats& getStats() const { return stats_; }
