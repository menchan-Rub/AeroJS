/**
 * @file node.h
 * @brief AeroJS Abstract Syntax Tree (AST) の基底ノードクラス定義
...
#include <string>
#include <variant> // Potential use for values or types
#include <optional>
#include <cstdint>
#include <stdexcept> // For specific exceptions like std::bad_alloc if necessary
#include <nlohmann/json_fwd.hpp> // Forward declaration for JSON serialization
...
class AstVisitor; // Base class for AST visitors
class ConstAstVisitor; // Base class for const AST visitors
class StatementNode;
...
enum class NodeType : uint16_t {
    ...
    // --- Statements ---
    ExpressionStatement, ///< 式文 (e.g., console.log("hello");)
    IfStatement,        ///< if文
    ...
    // --- Expressions ---
    Identifier,         ///< 識別子 (variable, function name)
    PrivateIdentifier,  ///< プライベート識別子 (#field)
    Literal,            ///< リテラル (string, number, boolean, null, regexp, bigint)
    ...
    BinaryExpression,   ///< 二項演算子式 (+, -, *, /, ==, ===, in, instanceof, etc.)
    LogicalExpression,  ///< 論理演算子式 (&&, ||, ??)
    AssignmentExpression, ///< 代入式 (=, +=, -=, etc.)
    ...
    TemplateElement,    ///< テンプレートリテラル要素 (string part, ${ part })
    ...
    TsNonNullExpression,///< Non-null assertion (expr!)
...
using ConstNodePtr = std::unique_ptr<const Node>; // Usually not needed as unique_ptr is const-propagating
