...
Object::Object(Context* ctx)
    : m_context(ctx), m_prototype(nullptr), m_properties([](const PropertyKey& key) { return key.hash(); }, [](const PropertyKey& a, const PropertyKey& b) { return a == b; }), m_objectClass(ObjectClass::Base), m_extensible(true), m_sealed(false), m_frozen(false), m_internalSlots(), m_customData(), m_refCount(1) {
  // オブジェクト作成時のメモリ使用量を追跡
...
Value* Object::toPrimitive(PrimitiveHint hint) const {
  ...
  const std::vector<std::string> methods =
      (hint == PrimitiveHint::String) ? std::vector<std::string>{"toString", "valueOf"} : std::vector<std::string>{"valueOf", "toString"};
